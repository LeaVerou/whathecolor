...
/**
 * Build an HTML syntax hint for a CSS color format, e.g. `oklch(<percentage>[0,100] <number> <hue>)`.
 * Highlights the coordinate type the user is currently using when known.
 *
 * @param {object} options
 * @param {object} [options.meta] - Parse metadata from `Color.parse(str, {meta})`
 * @param {Color} [options.color] - Parsed color (used to resolve its color space)
 * @param {string} [options.formatId] - Format/function name (e.g. "oklch") when there's no valid color yet
 * @returns {string} HTML markup for the hint, or "" if no hint applies
 */
